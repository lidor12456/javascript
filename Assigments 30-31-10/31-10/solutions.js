...
const displayDataContainer = document.querySelector(".display-data")
const costumerContainer = document.querySelector(".costumers-container")
const costumersUl = document.querySelector("#costumers-ul")
const costumersBtn = document.querySelector("#costumers-btn")

const card = document.querySelector(".card")
const cardName = document.querySelector(".name")
const cardId = document.querySelector(".id")
const cardCash = document.querySelector(".cash")
...
const getAllCustomersDataById = async (obj) => {

    id = obj.classList.value;
    data = await fetchData(`https://capsules7.herokuapp.com/api/carMarket/customers/${id}`);
    console.log(data);
    return data;
}

costumersUl.addEventListener('click', async (e) => {
    data = await getAllCustomersDataById(e.target)
    cardName.textContent = `Name : ${data.name} `;
    cardId.textContent = `Id : ${data.id} `;
    cardCash.textContent = `Cash : ${data.cash} `;
})


const getCustomersByClick = async (res) => {
    res = startApp();
    res = await res;
    res.customers.forEach(e => {
        const div = document.createElement("div");
        div.classList.add(`${e.id}`)
        div.textContent = e.name; 
         costumersUl.appendChild(div);
        costumerContainer.appendChild(costumersUl) 
      });
      getAllCustomersDataById(costumersUl.children[0])
}
costumersBtn.addEventListener('click',getCustomersByClick);
...
// for (let i = 0; i < costumersUl.length; i ++) {
//     costumersUl[i].textContent = i;
// }


// div.addEventListener('click',getAllCustomersDataById);


// getCustomerByClick(startApp());
