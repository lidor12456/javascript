...
// getAgencyById(ourCarMarket.sellers,"gNHjNFL12");
...
const getAllAgenciesNameAndId = (agenciesArr) => {
  ...
  agenciesArr.forEach((element) => {
    let tempObj = {};
    tempObj.name = element.agencyName;
    tempObj.id = element.agencyId;
    sellersArr.push (tempObj);
...
const getCarsToBuyByModel = (agenciesArr, model) => {
  ...
  this.model= model;
  agenciesArr.forEach(element => {
    if (model in element.cars) {
      console.log(model);
    //!  newArr.push(element.cars.model)
    }
  });
  console.log(newArr);
return newArr; 
};
// getCarsToBuyByModel(ourCarMarket.sellers, "Bmw")
...
const getCustomerByName = (customersArr, name) => {
    ...
    customersArr.forEach(element => {
      if (element.name == name) {
        myCustomerArr.push (element);
  
      }
    });
    console.log(myCustomerArr);
    return myCustomerArr;
